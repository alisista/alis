...
Some hidden calls not specified in the document are also revealed with this library such as `comments` and `notifications`. To see the full list of the available calls, please see the source code [here](https://github.com/alisista/alis/blob/master/lib/apis.js).

All you have to do to make a call is to replace the `slashes` with `dots` and remove `{}` from the pathname of the call. Also specify the `request method` in the second argument when it's not `GET`.

Your `username` and `password` go to the second argument as well when authentication is needed for the call.

Some examples.

[GET] /articles/{article_id}
```js
alis.articles.article_id((err, json, obj)=>{})
```
[POST] /me/articles/{article_id}/like
```js
alis.me.articles.article_id.like({article_id: `2xANm0jEzLPB`}, {method: `post`, username: `your_username`, password: `your_password`}, (err, json, obj)=>{})
```
