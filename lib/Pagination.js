...
class Response{
  constructor(opts = {}, conf = {}, json, url, cb){
    ...
    if(this.itemCount === 0){
      ...
      this.startNth = (opts._endNth == undefined) ? 1 : (opts._endNth + 1)
...
class Pagination{
  static pagination(opts = {}, conf = {}, cb){
	...
	if(apis[conf.path_name][conf.method || `get`].pagination !== true){
	  ...
	  if(typeof(conf.getAll) === `function`){
	    conf.getAll((pagination.json || null), pagination.next, pagination.stop, pagination)
	  }else{
  ...
  static async paginationP(opts = {}, conf = {}){
	...
	isStop = await new Promise((res, rej)=>{
	  conf.getAll(pagination.json, res, ()=>{
	    res({stop:true})
	  }, pagination)
	})
	if(isStop !== true){
